...
public class GUI_PhieuMuon extends JFrame implements ActionListener, MouseListener{
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					GUI_PhieuMuon frame = new GUI_PhieuMuon();
	...
	public GUI_PhieuMuon() {
		...
		JLabel lbl_TieuDe = new JLabel("TẠO PHIẾU MƯỢN");
...
//		panel_2.add(txtField_SoLuong);
		...
		scroll.setBounds(20, 380, 550, 150);
		panel_2.add(scroll);
...
//		SLa = txtField_SoLuong.getText();
		...
		if (o.equals(btn_xacnhan)) {
			if(valid()) {
				...
				PhieuMuon phieuMuon = new PhieuMuon(ngayMuon, ngayTra, trangThai, thuThu, docGia);
				PhieuMuonController muonController = new PhieuMuonController();
				String phieuMuonId = muonController.addPhieuMuon(phieuMuon);
				phieuMuon = muonController.getPhieuMuonById(phieuMuonId);
				if(phieuMuonId != null && phieuMuon != null) {
					...
					for(int i = 0; i < listSach.size(); i++) {
						...
						chiTietPhieuMuonController.addChiTietPhieuMuon(chiTietPhieuMuon);
					}
					JOptionPane.showMessageDialog(this, "Thêm phiếu mượn thành công!");
					clearData();
				}else {
					JOptionPane.showMessageDialog(this, "Thêm phiếu mượn thất bại! Vui lòng thử lại");
				}
			}
